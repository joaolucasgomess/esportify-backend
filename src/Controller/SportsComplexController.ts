...
export class SportsComplexController {
    ...
    addSportsComplex = async(req: Request, res: Response): Promise<void> => {
        try{
            const { cnpj, nome, rua, bairro, cidade, complemento, numero, cep } = req.body
            const new_sports_complex =  await this.sportsComplexBusiness.addSportsComplexBusiness({ cnpj, nome, rua, bairro, cidade, complemento, numero, cep })
