...
export class CourtService {
	...
	addCourt = async (newCourt: Omit<NewCourt, "id">): Promise<Court> => {
		try {
			...
			const courtByNameAndIdSportsComplex =
				await this.courtRepository.selectCourtByNameAndSportsComplexId(
					newCourt.name,
					newCourt.sportsComplexId,
				);

			if (courtByNameAndIdSportsComplex) {
				throw new CustomError(
					"Quadra já cadastrada no complexo esportivo informado",
					422,
				);
			}
			...
			const court = await this.courtRepository.insertCourt({
				id,
				name: newCourt.name,
		...
		try {
			if (sportsComplexId) {
				...
				if (!courtsBySportsComplex) {
					throw new CustomError(
						"Este complexo esportivo ainda não possui quadras",
						404,
					);
				}
				...
				const allCourts =
					await this.courtRepository.selectAllCourts(querys);
	...
	getCourtById = async (
		...
		try {
			...
			if (
				sportesComplexId &&
				courtById.sportsComplexId !== sportesComplexId
			) {
				throw new CustomError(
					"Seu usuário não possui acesso a esta quadra",
					403,
				);
			}
