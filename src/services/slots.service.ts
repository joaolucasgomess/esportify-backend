...
export class SlotsService {
		...
		try {
			...
			if (newSlot.price <= 0) {
				throw new CustomError(
					"O preço informado é menor ou igual a zero",
					422,
				);
			}
			...
			const conflictedSlot =
				await this.slotsRepository.findCoflictedSlots({
	...
	deleteSlot = async (
		...
		try {
			...
			const slotToDelete =
				await this.slotsRepository.selectSlotById(slotId);
	...
	getSlotsByCourtId = async (courtId: string): Promise<AvailableSlot[]> => {
		try {
			...
			if (slotsByIdCourt.length === 0) {
				throw new CustomError(
					"Quadra não possui horários cadastrados",
					404,
				);
			}
